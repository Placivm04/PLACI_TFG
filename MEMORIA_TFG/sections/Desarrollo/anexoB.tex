Este apéndice describe los pasos necesarios para obtener el código fuente del proyecto, preparar el entorno de ejecución y repetir los experimentos presentados.
...
\begin{itemize}
    \item \texttt{DATASETS/}: contiene el script de preprocesamiento en R y los conjuntos de datos reducidos comprimidos empleados por los notebooks.
...
\begin{itemize}
    \item \texttt{MODELOS\_IT/}: entrenamiento y comparación de modelos sobre UNSW-NB15.
    \item \texttt{MODELOS\_IOT/}: entrenamiento y comparación de modelos sobre IoT-23.
    \item \texttt{MODELOS\_TON\_IOT/}: entrenamiento y comparación de modelos sobre ToN-IoT.
...
\begin{verbatim}
git clone https://github.com/Placivm04/PLACI_TFG.git PROYECTO_TFG
cd PROYECTO_TFG
\end{verbatim}

Si el código se entrega como un archivo comprimido, basta con descomprimirlo y abrir una terminal en la carpeta raíz del proyecto

\section{Preparación del entorno Python}

Para reproducir los experimentos se recomienda crear un entorno virtual propio desde la raíz del repositorio e instalar en él las dependencias incluidas en \texttt{requirements.txt}. En el entorno utilizado para esta memoria la versión de Python era \texttt{3.12.3}.

\begin{verbatim}
python3 -m venv CODIGO/.venv
...
En sistemas Windows, la activación del entorno se realiza con:

\begin{verbatim}
CODIGO\.venv\Scripts\activate
\end{verbatim}

El comando de instalación leerá el fichero \texttt{requirements.txt} e instalará las librerías necesarias para ejecutar el código, como Optuna, TensorFlow, Scikit-learn, entre otras.

\section{Preparación de los datos}

Las notebooks del proyecto trabajan con los datos reducidos comprimidos situados en:

\texttt{DATASETS/dataSets\_Reducidos/}

Para descomprimirlos, basta con ejecutar:

\texttt{find . -name "*.csv.gz" -type f -exec gzip -d \{\} \textbackslash;}

Obtendríamos los siguientes ficheros CSV listos para ser usados por las notebooks:
...
\begin{itemize}
    ...
    \item \texttt{DATASETS/dataSets\_Reducidos/iot-23/datos\_IOT\_23\_1\_1\_redux.csv}
    \item \texttt{DATASETS/dataSets\_Reducidos/iot-23/datos\_IOT\_23\_3\_1\_redux.csv}
    \item \texttt{DATASETS/dataSets\_Reducidos/ton\_iot/datos\_TON\_IoT\_redux.csv}
\end{itemize}

Como estos ficheros ya están presentes, no es necesario ejecutar el preprocesamiento en R. En ese caso se puede pasar directamente a la ejecución de las notebooks.

Si se desea reproducir el proceso desde los datos originales, hay que colocar los ficheros originales (obtenidos desde las páginas web de los datasets) en las rutas esperadas por \texttt{DATASETS/Preprocesamiento.r}. 
Las capturas necesarias vienen detalladas en la sección \ref{datasets}. Una vez corregidas las rutas absolutas, se ejecuta:

\begin{verbatim}
Rscript DATASETS/Preprocesamiento.r
\end{verbatim}

Este paso genera los ficheros reducidos en \texttt{DATASETS/dataSets\_Reducidos/}. Para IoT-23, la notebook \texttt{CODIGO/MODELOS\_IOT/rf\_train\_iot.ipynb} también contiene celdas de preparación que construyen el fichero final \texttt{datos\_IOT\_23\_preparado.csv} a partir de las capturas reducidas.
...
Para UNSW-NB15 se usan las notebooks de \texttt{CODIGO/MODELOS\_IT/}:
...
Para IoT-23 se usan las notebooks de \texttt{CODIGO/MODELOS\_IOT/}:
...
Para ToN-IoT se usan las notebooks de \texttt{CODIGO/MODELOS\_TON\_IOT/}:
