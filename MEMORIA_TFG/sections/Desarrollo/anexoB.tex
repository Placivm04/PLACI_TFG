...
\begin{itemize}
    ...
    \item Jupyter Notebook o JupyterLab para ejecutar las notebooks.
\end{itemize}
...
Los experimentos de entrenamiento se organizan por \textit{dataset}. En cada carpeta hay una notebook por familia de modelos. El procedimiento general es:
...
\begin{enumerate}
    ...
    \item Seleccionar el kernel \texttt{TFG IDS}.
...
\begin{itemize}
    ...
    \item \texttt{lstm\_iot\_train.ipynb}
...
Estas notebooks cargan los \texttt{joblib}, preparan los datos de test y calculan métricas como \textit{accuracy}, F1, latencia de inferencia y consumo de recursos. Esta es la vía más rápida para comprobar los resultados finales sin repetir la búsqueda completa de hiperparámetros.
...
También existen versiones con prefijo \texttt{for\_ataques}, usadas como notebooks auxiliares para preparar y comprobar la carga de modelos antes de ejecutar las pruebas completas.
...
Los ataques se generan principalmente sobre modelos neuronales y después se evalúa la degradación en distintos clasificadores. Las métricas que deben compararse con las recogidas en la memoria son \textit{accuracy}, F1 y \textit{recall}. Las figuras finales incluidas en la memoria se encuentran en:

\begin{itemize}
    \item \texttt{MEMORIA\_TFG/images/ATAQUES\_ADVERSARIOS/}
    \item \texttt{MEMORIA\_TFG/images/ATAQUES\_ADVERSARIOS\_2/}
\end{itemize}

\section{Compilación de la memoria}

Para compilar la memoria hay que situarse en \texttt{MEMORIA\_TFG/}. El documento principal es \texttt{main.tex}. La plantilla usa \texttt{fontspec}, por lo que debe compilarse con XeLaTeX o LuaLaTeX. Una secuencia habitual de compilación es:
...
La plantilla utiliza recursos locales de \texttt{template/}, imágenes de \texttt{images/} y la bibliografía situada en \texttt{sections/Referencias/references.bib}. Si el compilador informa de referencias o citas pendientes, se debe repetir \texttt{xelatex main.tex}.
...
\begin{itemize}
    \item Que las notebooks cargan los \texttt{csv} desde \texttt{DATASETS/dataSets\_Reducidos/} sin errores de ruta.
    \item Que el kernel de Jupyter corresponde al entorno donde se instalaron las dependencias.
    \item Que las notebooks de comparación muestran métricas de F1 y \textit{accuracy} similares a las tablas de la memoria.
    \item Que los tiempos de inferencia tienen el mismo orden relativo entre modelos, aunque no coincidan exactamente con los de la memoria.
    \item Que las notebooks de ataques muestran una degradación de prestaciones al aumentar la intensidad de FGSM o PGD.
\end{itemize}
