En este apéndice se presenta el análisis detallado de los resultados obtenidos durante la fase de optimización de hiperparámetros para cada modelo y \textit{dataset}.
...
Hemos seleccionado las siguientes combinaciones de la frontera de Pareto para testarlas:
...
Los resultados obtenidos en el \textit{dataset} de test son los siguientes:
...
La frontera de Pareto obtenida para CatBoost con el \textit{dataset} UNSW-NB15 es la siguiente:
...
Dentro de las limitaciones del enfoque lineal, el \textbf{Candidato 3} (C=0.000187) es el ganador.
...
Seleccionamos el \textbf{Candidato 4}, que ofrece el mejor rendimiento (F1-Score de 0.7869) y una latencia de inferencia de 0.001421 ms, lo que lo posiciona como un modelo competitivo dentro de los modelos no lineales, aunque aún por debajo de los basados en árboles.
...
Aunque su rendimiento es competitivo, su latencia es significativamente mayor que la de los modelos basados en árboles, lo que podría limitar su aplicabilidad en entornos de producción donde la velocidad de detección es crítica.
...
En este caso, el \textbf{Candidato 1} es el modelo más eficiente, con un F1-Score de 0.999675 y una latencia de inferencia de 0.001496 ms, lo que lo posiciona como un modelo preciso y rápido para este \textit{dataset} específico de IoT.
...
En este caso, el \textbf{Candidato 2} es el modelo más eficiente, con un F1-Score de 0.999122 y una latencia de inferencia de 0.000027 ms, lo que lo posiciona como un modelo preciso y rápido en este \textit{dataset} de IoT, superando incluso a los modelos de Random Forest en términos de latencia.
...
En este caso, el \textbf{Candidato 1} es el mejor modelo, con un F1-Score de 0.999813 y una latencia de inferencia de 0.000057 ms.
...
En este caso, el \textbf{Candidato 2} es el modelo más eficiente, con un F1-Score de 0.999960 y una latencia de inferencia de 0.000256 ms, lo que lo posiciona como un modelo bastante bueno.
...
En este caso, el \textbf{Candidato 2} es el modelo más eficiente, con un F1-Score de 0.943695 y una latencia de inferencia de 0.000025 ms, lo que lo posiciona como un modelo muy bueno para este \textit{dataset} de IoT, aunque su rendimiento es significativamente inferior al de los modelos basados en árboles.
...
En este caso, nos quedamos con el \textbf{Candidato 1}, que ofrece un excelente rendimiento (F1-Score de 0.994819) y una latencia de inferencia de 0.000717 ms, lo que lo posiciona como un modelo muy bueno para este \textit{dataset} de IoT, aunque su rendimiento sigue siendo inferior al de los modelos basados en árboles.
...
En este caso, el \textbf{Candidato 3} es el modelo más eficiente, con un F1-Score de 0.995359 y una latencia de inferencia de 0.00892 ms, lo que lo posiciona como un modelo muy bueno para este \textit{dataset} de IoT, aunque su rendimiento sigue siendo inferior al de los modelos basados en árboles.

\section{Modelos entrenados con TON-IoT}

\subsection{Random Forest}

La frontera de Pareto obtenida para Random Forest con el \textit{dataset} ToN-IoT es la siguiente:
...
El \textbf{Candidato 2} representa el punto de equilibrio perfecto entre capacidad de detección y coste computacional.

\subsection{XGBoost}

La frontera de Pareto obtenida para XGBoost con el \textit{dataset} ToN-IoT es la siguiente:
...
La frontera de Pareto obtenida para LightGBM con el \textit{dataset} ToN-IoT es la siguiente:
...
Nos quedamos con el \textbf{Candidato 6} que ofrece un excelente rendimiento (F1-Score de 0.997085) y una latencia de inferencia de 0.000202 ms, lo que lo posiciona como un modelo muy bueno para este \textit{dataset} de IoT, aunque su rendimiento es inferior comparado con los otros ensembles de árboles.

\subsection{CatBoost}

Para CatBoost, la frontera de Pareto obtenida con el \textit{dataset} ToN-IoT es la siguiente:
...
El \textbf{Candidato 4} representa el punto de equilibrio perfecto entre capacidad de detección y coste computacional, con un F1-Score de 0.996628 y una latencia de inferencia de 0.000354 ms.

\subsection{SVM}

La frontera de Pareto obtenida para SVM con el \textit{dataset} ToN-IoT es la siguiente:
...
El \textbf{Candidato 4} representa el punto de equilibrio perfecto entre capacidad de detección y coste computacional, con un F1-Score de 0.897815 y una latencia de inferencia de 0.000031 ms, lo que lo posiciona como un modelo bastante bueno para este \textit{dataset} de IoT, aunque su rendimiento es significativamente inferior al de los modelos basados en árboles.

\subsection{MLP}

La frontera de Pareto obtenida para MLP con el \textit{dataset} ToN-IoT es la siguiente:
...
El \textbf{Candidato 4} representa el punto de equilibrio perfecto entre capacidad de detección y coste computacional, con un F1-Score de 0.963034 y una latencia de inferencia de 0.002268 ms, lo que lo posiciona como un modelo bastante bueno para este \textit{dataset} de IoT, aunque su rendimiento sigue siendo inferior al de los modelos basados en árboles.

\subsection{CNN-1D}

La frontera de Pareto obtenida para CNN-1D con el \textit{dataset} ToN-IoT es la siguiente:
